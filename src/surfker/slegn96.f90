...
! model param
integer              :: mmax
real(kind=dp),ALLOCATABLE  :: zd(:),zb(:),zrho(:),za(:),xmu(:)
integer,ALLOCATABLE  :: iwat(:)

! eigen function
real(kind=dp),ALLOCATABLE  :: uu(:),tt(:),dcdb(:),dcdh(:),dcdr(:)
real(kind=dp)              :: uu0(4)
...
! save
real(kind=dp),ALLOCATABLE  :: exl(:)

! love wave var
real(kind=dp)              :: cosq,sinq,yl,zl,mu

! sumi
real(kind=dp)              :: sumi0,sumi1,sumi2,flagr,ale,ugr
...
subroutine bldsph()
  ...
  implicit none
  integer                :: i
  real(kind=dp)                :: ar, dr, r0, r1, z0, z1,tmp
  !c-----
...
subroutine shfunc(omega,wvno)
  ...
  implicit none
  real(kind=dp)        :: fl,omega,wvno

  ! local
  real(kind=dp)        :: ext,umax,fact
  integer        :: k
  !c-----
...
subroutine varl(m,rb,omega,wvno,xkb,dpth,eexl)
  ...
  implicit none 
  integer m
  real(kind=dp)            ::  rb, xkb, omega, wvno, dpth, eexl,q

  real(kind=dp)            :: wvnop, wvnom , fac
...
subroutine emat(xnub,wvno,el,einvl)
  ...
  !c-----
  real(kind=dp)              :: wvno
  complex(kind=dp)   :: xnub, el(2,2), einvl(2,2)
...
subroutine up(omega,wvno,fl)
  ...
  real(kind=dp)        :: fl,omega,wvno
  real(kind=dp)        :: a11, a12, a21, a22,rb, xkb, dpth,hl(2,2)
  real(kind=dp)        :: eexl,amp0,str0,rr,ss,ttlast
  integer        :: mmx1,k,k1
...
subroutine energy(omega,wvno,&
  ...
  implicit none
  complex(kind=dp)     :: nub,xnub,exqq,f1,f2,f3
  real(kind=dp)                :: omega,wvno
  COMPLEX(kind=dp)     :: CDEXP,kmup, km1dn
  complex(kind=dp)     :: einvl(2,2), el(2,2)
  complex(kind=dp)     :: f, g
  real(kind=dp)                :: TN, TL,Eut,Edut,Ed2ut,Eut0,Ett0,upup,dupdup
  real(kind=dp)                :: DCDBH, DCDBV, DCDRSH,dcr,dcb,fac,&
                                    c2,llflag,dvdz,dfac
  real(kind=dp)                :: VSHH, VSHV,c,omega2,wvno2,drho,dpth,&
                                    dmu,rb,xkb,eexl
  integer                :: k,k1,LSS,lrr
...
subroutine splove(om,c,csph,usph,ugr)
  ...
  real(kind=dp)                :: om,tm,c,csph,usph
  real(kind=dp)                :: ugr,a
  INTEGER                :: i 
  a = 6371.0d0
...
subroutine slegn96(thk,vs,rhom,nlayer,&
  ...
  use LoveWaveModel
  implicit none
  integer,value,INTENT(IN)   ::nlayer,iflsph
  real(kind=cr),INTENT(IN)          :: thk(nlayer),vs(nlayer),rhom(nlayer)
  real(kind=dp),INTENT(IN)         :: t 
  real(kind=dp),INTENT(INOUT)      :: cp,cg 
  real(kind=dp),INTENT(INOUT)      :: disp(nlayer),stress(nlayer),&
                                       dc2db(nlayer),dc2dh(nlayer),&
                                       dc2dr(nlayer)
  
  ! local variables
  integer           :: i,nsph
  real(kind=dp)           :: c, omega, wvno, gamma, csph, usph,twopi
  real(kind=dp)           :: Eut, Edut, Eut0, Ett0, Ed2ut
...
subroutine slegnpu(thk,vs,rhom,nlayer,&
  ...
  real(kind=dp),intent(inout)    :: du2db(nlayer),du2dh(nlayer),du2dr(nlayer)
  ...
  real(kind=dp),ALLOCATABLE  :: dc2db1(:),dc2db2(:),&
